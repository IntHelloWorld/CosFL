...
def load_callgraph(graphml_files):
    ...
    for graphml_file in graphml_files:
        ...
        for u, v, data in raw_G.edges(data=True):
            new_G.add_edge(get_node_from_str(u), get_node_from_str(v), weight=int(data['weight']))
    return new_G
...
def cluster_graph(G: nx.DiGraph, min_size: int = 5, max_size: int = 15):
    ...
    community_sizes = {cluster_id: clusters.count(cluster_id) for cluster_id in set(clusters)}
    ...
    for small_comm in small_communities:
        ...
        small_comm_nodes = [i for i, cluster in enumerate(clusters) if cluster == small_comm]

        # Find the nearest larger community
        nearest_comm = None
        max_connections = 0
        for target_comm in set(clusters):
            if target_comm != small_comm and community_sizes[target_comm] >= min_size:
                weights = []
                for node in small_comm_nodes:
                    for neighbor in ig_graph.neighbors(node):
                        if clusters[neighbor] == target_comm:
                            try:
                                weights.append(
                                    ig_graph.es[ig_graph.get_eid(node, neighbor)]["weight"]
                                )
                            except:
                                weights.append(
                                    ig_graph.es[ig_graph.get_eid(neighbor, node)]["weight"]
                                )
...
def prepare_method_summarization_input(method_node, method_context_node):
    ...
    """
    context_text = "Unavailable" if method_context_node is None else method_context_node.text
    input_text = f"\nMethod Code:\n\n{method_node.text}\n"
